...
export default function PantallaResultados({ route, navigation }) {
  ...
  const cargarVocacional = useCallback(async () => {
    try {
      ...
      const idACargar = route.params?.usuarioId || id;
      console.log('🎓 Cargando resultados vocacional para ID:', idACargar);

      let respuesta;
      if (idACargar === id) {
        // Es el propio usuario
        respuesta = await servicioAPI.obtenerResultadosVocacionales();
      } else {
        // Es otro usuario
        respuesta = await servicioAPI.obtenerResultadosVocacionalesPorUsuario(idACargar);
      }

      if (respuesta.exito && respuesta.data && Array.isArray(respuesta.data)) {
        // Filtrar por el usuario que corresponde (por si la API devuelve varios)
        const resultadosUsuario = respuesta.data.filter(item =>
          item.user_id === idACargar ||
          item.usuario_id === idACargar
        );
  ...
  }, [obtenerUsuarioActual, route.params?.usuarioId]);
  ...
  const cargarTestsConocimiento = useCallback(async () => {
    try {
      ...
      // Determinar qué usuario cargar
      const idACargar = route.params?.usuarioId || id;
      console.log('📚 Cargando tests de conocimiento para ID:', idACargar);

      // 1. Obtener tests disponibles (siempre los mismos)
      ...
      // Filtrar solo tests de conocimiento (excluir vocacional/Ikigai)
      ...
      // 2. Obtener resultados del usuario correspondiente
      let respuesta;
      if (idACargar === id) {
        // Propio usuario
        respuesta = await servicioAPI.obtenerMisResultados();
      } else {
        // Otro usuario
        respuesta = await servicioAPI.obtenerResultadosTestsPorUsuario(idACargar);
      }

      let resultadosUsuario = [];
      if (respuesta.exito && Array.isArray(respuesta.data)) {
        resultadosUsuario = respuesta.data.filter(item =>
          item.user_id === idACargar ||
          item.usuario_id === idACargar
        );
      }

      // Mapear tests con resultados
      ...
      <SafeAreaView style={styles.contenedor}>
        ...
        <View style={styles.encabezado}>
          <Text style={styles.tituloPrincipal}>
            {route.params?.usuarioId && route.params?.usuarioId !== usuarioActualId
              ? `Resultados de ${nombreUsuario || 'Usuario'}`
              : 'Mis Resultados'}
          </Text>
          <Text style={styles.subtitulo}>
            {pestañaActiva === 'vocacional'
              ? (resultadosVocacional ? 'Test vocacional completado' : 'Test vocacional pendiente')
              : `${resultadosTests.filter(t => t.completado).length} de ${resultadosTests.length} tests completados`}
          </Text>
        </View>
